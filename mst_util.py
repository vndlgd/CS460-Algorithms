...
class MinPriorityQueue(Generic[T]):
    ...
    def has(self, elem: T) -> bool:
        return elem in [it[0] for it in self.heap]
...
class DisjointSet:
    def __init__(self, size):
        ...
        for i in range(size):
            self.parent[i] = i # Make all nodes his own parent, i.e., creating n sets.

    def print_set(self):
        s = 'size: ' + ','.join(map(str, self.size)) + '\nparent: ' + ','.join(map(str, self.parent))
        print(s)

    def merge_set(self, node1, node2):
        """
        Args:
            node1, node2 (int): Indices of nodes whose sets will be merged.
        """
    ...
    def find_set(self, node):
        """
        Get the root element of the set containing a node
        :param node: index of node whose root to be found
        :return:
        """
